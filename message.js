...
ptz.sendMessage(m.chat, Messagedoc, {quoted: m })
}
...
ptz.sendMessage(m.chat, { caption: kontol}),{quoted : floc}
}
break
                        ...
                        //let img = await q.download()
                            let img = await ptz.downloadAndSaveMediaMessage(quoted, makeid(5))
                                if (!img) throw 'Reply a sticker!'
                                    var stiker = await addExifAvatar(img, `Sakia`, `${pushname}`)
                                      } catch (e) {
                                          console.error(e)
